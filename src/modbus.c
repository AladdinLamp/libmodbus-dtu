...
static int send_msg_raw(modbus_t *ctx, uint8_t *msg, int msg_length) {
    int rc;
    int i;
...
int serial_send_raw_request(modbus_t *ctx, const uint8_t *raw_req, int raw_req_length)
{
    if (ctx == NULL) {
        errno = EINVAL;
        return -1;
    ...
    return send_msg_raw(ctx, raw_req, raw_req_length);
}
...
int serial_receive_msg(modbus_t *ctx, uint8_t *msg, int max_length)
{
    int rc;
    fd_set rset;
    struct timeval tv;
	...
	rc = ctx->backend->recv(ctx, msg, max_length);
	if (rc == 0) {
		errno = ECONNRESET;
		rc = -1;
	}
...
char check_645_finish(uint8_t *msg, int msg_length)
{
...
int receive_645_msg(modbus_t *ctx, uint8_t *msg, int max_length)
{
    int rc;
    fd_set rset;
    struct timeval tv;
	...
	while (finish == 0) { 
		...
		rc = ctx->backend->recv(ctx, msg + msg_length, max_length);
		if (rc == 0) {
			errno = ECONNRESET;
			rc = -1;
		}
		...
		if (rc > 0) { 
			msg_length += rc;
			finish = check_645_finish(msg, msg_length);
		}
...
int modbus_read_func_registers(modbus_t *ctx, int function, int addr, int nb, uint16_t *dest)
{
    int status;
    ...
    status = read_registers(ctx, function,
                            addr, nb, dest);
    return status;
}
